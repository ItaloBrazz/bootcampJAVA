...
> 🎯 **Ideal para:** Estudantes aprendendo Spring Boot, APIs REST, JPA, desenvolvimento front-end e integração entre frontend e backend
...
#### **`model/Produto.java`**
- Entidade JPA com atributos: id, nome, preço
...
- Configurações da aplicação (porta, banco de dados, JPA)
...
```bash
curl -X POST http://localhost:8085/produtos \
...
#### 2. Listar produtos
```bash
curl http://localhost:8085/produtos
```

#### 3. Buscar por ID
```bash
curl http://localhost:8085/produtos/1
```

#### 4. Atualizar produto
```bash
curl -X PUT http://localhost:8085/produtos/1 \
  -H "Content-Type: application/json" \
...
#### 5. Deletar produto
```bash
curl -X DELETE http://localhost:8085/produtos/1
```
...
1. Acesse: `http://localhost:8085/h2-console`
...
|---------|-----------|-------------|
| 🗄️ **H2 Database** | Banco de dados em memória | `jdbc:h2:mem:produtosdb` |
| 🔧 **H2 Console** | Interface web para visualizar dados | `http://localhost:8085/h2-console` |
| 📝 **JPA/Hibernate** | ORM para mapeamento objeto-relacional | Automático via Spring Data JPA |
| 🔄 **Auto DDL** | Criação automática de tabelas | `create-drop` (reinicia a cada execução) |
...
> ⚠️ **Nota:** O `create-drop` faz com que as tabelas sejam recriadas a cada execução, perdendo os dados. Para persistência, altere para `update` ou `none` no `application.properties`.
...
- **Padrões** - MVC, Repository, Dependency Injection
...
**Dependências:**
- Spring Boot 4.0.2, Spring Web MVC, Spring Data JPA, H2 Database, Hibernate
...
#### Erro: "Porta 8085 já está em uso"
```bash
# Windows
...
- Confirme que está rodando na porta 8085
- Verifique os logs no console

### Frontend

#### Erro: "Erro ao carregar produtos"
- Verifique se a API está rodando em `http://localhost:8085`
...
- Teste o endpoint: `http://localhost:8085/produtos`

### Geral

#### Ambos não funcionam juntos
- Sempre inicie o backend antes do frontend
- Backend (8085) e Frontend (8000) devem estar em portas diferentes
